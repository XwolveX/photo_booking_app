...
class _BookingStep1ScreenState extends State<BookingStep1Screen>
  ...
  void initState() {
    ...
    final initialTab =
    (widget.preSelectedMakeuper != null && widget.preSelectedPhotographer == null)
        ? 1
        : 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        if (widget.preSelectedPhotographer != null ||
            widget.preSelectedMakeuper != null)
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Row(
            children: [
              _stepCircle(1, activeStep, false, isDark),
              Expanded(child: _stepConnector(1 < activeStep, isDark)),
              _stepCircle(2, activeStep, false, isDark),
              Expanded(child: _stepConnector(2 < activeStep, isDark)),
              _stepCircle(3, activeStep, false, isDark),
              Expanded(child: _stepConnector(3 < activeStep, isDark)),
          ...
          Row(
            children: [
              _stepLabel(labels[0], 1 == activeStep, isDark),
  ...
  Widget _buildPreSelectedBanner(bool isDark) {
    final provider =
        widget.preSelectedPhotographer ?? widget.preSelectedMakeuper!;
  ...
  Widget _buildProviderList(bool isDark) {
    return TabBarView(
      ...
      children: [
        _buildRoleList(
            'photographer', isDark, _photoScrollCtrl, _selectedPhotographer),
        _buildRoleList(
            'makeuper', isDark, _makeupScrollCtrl, _selectedMakeuper),
      ],
    );
  }

  Widget _buildRoleList(String role, bool isDark,
      ScrollController scrollCtrl, Map<String, dynamic>? selected) {
    final isPhoto = role == 'photographer';
    final color =
    isPhoto ? AppTheme.rolePhotographer : AppTheme.roleMakeuper;
    final preSelectedForThisRole = isPhoto
        ? widget.preSelectedPhotographer
        : widget.preSelectedMakeuper;
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        if (preSelectedForThisRole != null) {
          sortedDocs.sort((a, b) =>
          a.id == preSelectedForThisRole['uid']
              ? -1
              : (b.id == preSelectedForThisRole['uid'] ? 1 : 0));
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, i) {
            ...
            final isThisPreSelected = preSelectedForThisRole != null &&
                preSelectedForThisRole['uid'] == uid;
            ...
            return _ProviderWithServicesCard(
              ...
              isLocked:
              preSelectedForThisRole != null && !isThisPreSelected,
              color: color,
              isDark: isDark,
              isPhoto: isPhoto,
              preSelectedServiceName: isThisPreSelected
                  ? preSelectedForThisRole['serviceName']
                  : null,
              onSelect: (price, name, sId, isDeselect) {
                if (preSelectedForThisRole != null) return;
                setState(() {
                  if (isPhoto) {
                    _selectedPhotographer = isDeselect
                        ? null
                        : {
                      ...data,
                      'uid': uid,
  ...
  double get _totalPrice {
    ...
    if (_selectedPhotographer != null)
      total +=
          (_selectedPhotographer!['price'] as num?)?.toDouble() ?? 0;
    if (_selectedMakeuper != null)
...
class _ProviderWithServicesCard extends StatelessWidget {
  ...
  final void Function(double? price, String? name, String? sId,
      bool isDeselect) onSelect;
                ...
                return Padding(
                  ...
                  child: Column(
                    children: docs.map((doc) {
                      ...
                      final sId = doc.id;
                      final bool isThisServiceSelected =
                          (isPreSelected &&
                              sName == preSelectedServiceName) ||
                            ...
                            child: Row(children: [
                              ...
                              Expanded(
                                  child: Text(sName,
                                      style: TextStyle(
                                          fontSize: 13,
                                          fontWeight: isThisServiceSelected
                                              ? FontWeight.bold
                                              : FontWeight.w500,
                                          color: isThisServiceSelected
                                              ? color
    ...
    return Padding(
      ...
      child: Row(children: [
        CircleAvatar(
            ...
            child: Icon(isPhoto ? Icons.camera_alt : Icons.brush,
                color: color, size: 20)),
        const SizedBox(width: 12),
        Expanded(
            child: Column(
